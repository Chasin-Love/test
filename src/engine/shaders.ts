...
uniform float uTear; uniform float uTearTime;
...
void main(){
  ...
  /* Embedded spatial core: object-space sphere deformation with radial
     compression, differential rotation, and multi-scale flow noise. */
  vec3 localCore = (inverse(modelMatrix) * vec4(uGravityCenter, 1.0)).xyz;
  vec3 coreDelta = position - localCore;
  ...
  float angularVelocity = (1.0 - smoothstep(0.03, 1.0, radiusNorm)) * 3.6 + 0.35;
  float angle = field * angularVelocity * (0.55 + 0.22 * sin(uGravityTime * 1.7 + coreDistance * 0.08));
  ...
  float suction = uTear * spot * (0.34 + 0.22 * (0.5 + 0.5 * wave));
  float shear = uTear * spot * (0.18 * wave + 0.08 * fracture);
  float rimKick = uTear * shell * fracture * 0.12;
...
uniform vec3 uVortexC; uniform float uVortexR; uniform float uVortexS; uniform float uVortexT; uniform float uVortexPull;
...
void main(){
  ...
  if (uVortexS > 0.001) {
    ...
    if (infl > 0.001) {
      ...
      float radialPull = infl * (0.5 + clamp(uVortexPull, 0.0, 1.0) * 0.5);
      vp = uVortexC + spun * max(0.035, 1.0 - radialPull);
      vAlpha *= (1.0 - infl * (0.6 + clamp(uVortexPull, 0.0, 1.0) * 0.3));
    }
  }
