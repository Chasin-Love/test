...
export class UniverseEngine {
  ...
  constructor(canvas: HTMLCanvasElement, bodies: CosmicBody[], cb: EngineCallbacks) {
    ...
    this.renderer.debug.onShaderError = (gl, program, _vs, fs) => {
      const source = (gl.getShaderSource(fs) ?? '').slice(0, 4000);
      const log = gl.getShaderInfoLog(fs) || gl.getProgramInfoLog(program) || 'unknown shader error';
      console.error('[universe] shader compile failure:', log);
      window.dispatchEvent(new CustomEvent('eventide-shader-error', { detail: { source, log } }));
    };
    ...
    this.renderer.debug.checkShaderErrors = false; /* production smoothness */
    ...
    if (data.kind === 'planet' || data.kind === 'dwarf') {
      const mat = new THREE.ShaderMaterial({
        uniforms: {
          ...
          uGravityCenter: { value: new THREE.Vector3() }, uGravityRadius: { value: 0 },
...
void main(){
  ...
  gl_FragColor = vec4(col, a);
    ...
    const astralCollider = new THREE.Mesh(
      new THREE.SphereGeometry(30000, 16, 14),
      new THREE.MeshBasicMaterial({ visible: false })
    );
  ...
  private buildInnerStellarSystem(gal: GalaxyData, galCol: THREE.Color, rnd: () => number, reality: RealityConfig): { root: THREE.Group; sys: InnerSystem } {
    ...
    sys.haloA = ringPts(starData.radius * 1.6, 700, [1, 0.82, 0.55], 0.42, 1.6);
    sys.haloB = ringPts(starData.radius * 1.9, 420, [0.55, 0.85, 0.8], -0.55, 1.3);
  ...
  private tick = () => {
    ...
    const ease = this.portal.phase === 'out'
      ? phaseProgress
    ...
    if (this.kamuiFlight !== null) {
      ...
      if (this.skyDomeMesh) this.skyDomeMesh.visible = true;
      if (this.farStarsPoints) {
        const farMat = this.farStarsPoints.material as THREE.ShaderMaterial;
        if (farMat.uniforms && farMat.uniforms.uOpacity) farMat.uniforms.uOpacity.value = 1;
      }
      this.skyNebulae.forEach((m) => { if (m.uniforms.uOpacity) m.uniforms.uOpacity.value = 1; });
  ...
  private updatePortalGravity() {
    ...
    const fieldRadius = Math.max(120, targetRadius * 180);
  ...
  private updateLevels(dt = 0) {
    ...
    /* the universe sky (backdrop dome, far stars, deep nebulae) belongs
       ...
       During the in-reality bend it stays VISIBLE (only softly dimmed) so
       the jutsu reveals the cosmos instead of erasing it. */
    ...
    if (this.farStarsPoints) {
      ...
      if (farMat && farMat.uniforms && farMat.uniforms.uOpacity) {
        farMat.uniforms.uOpacity.value = skyVisible ? Math.max(0.45, 1 - this.kamuiErase) : 0;
      }
    }
    this.skyNebulae.forEach((m) => {
      if (m.uniforms && m.uniforms.uOpacity) {
        m.uniforms.uOpacity.value = skyVisible ? Math.max(0.45, 1 - this.kamuiErase) : 0;
      }
